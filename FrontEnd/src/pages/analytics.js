...
async function initAnalytics() {
  ...
  const [budgetRes, targetRes] = await Promise.all([
    fetch(
      `https://api-lecho.vanix.shop/api/Analytics/budget?month=${month}&year=${year}`,
      {
        headers: { Authorization: "Bearer " + token },
      },
    ),
    fetch(
      `https://api-lecho.vanix.shop/api/Analytics/target?month=${month}&year=${year}`,
      {
        headers: { Authorization: "Bearer " + token },
      },
    ),
...
window.resetMonth = async function () {
  ...
  await fetch(
    `https://api-lecho.vanix.shop/api/Analytics/reset?month=${month}&year=${year}`,
    {
      method: "POST",
      headers: {
